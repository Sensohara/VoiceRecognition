...
def run_ctc():
    ...
    with graph.as_default():
        ...
        inputs = tf.placeholder(tf.float32, [None, None, num_features], name="input_features")

        # Here we use sparse_placeholder that will generate a
        # SparseTensor required by ctc_loss op.
        targets = tf.sparse_placeholder(tf.int32, name="output")

        # 1d array of size [batch_size]
        seq_len = tf.placeholder(tf.int32, [None], name="input_seq_len")
        ...
        optimizer = tf.train.MomentumOptimizer(learning_rate=0.001, momentum=0.9, name="optimizer").minimize(cost)
